...
// prompt di api/chat.js (chatbot website), TIDAK mengubahnya.
...
async function generateEvaReply({
  ...
  if (!apiKey) {
    return { reply: '', model: '', error: 'GEMINI_API_KEY belum diisi' };
  }

  // Susun contents: riwayat (lama -> baru) + pesan terbaru.
  const turns = [];
  let totalChars = 0;
  for (const turn of history) {
    const text = String(turn && turn.text ? turn.text : '').trim();
    if (!text) continue;
    turns.push({ role: turn.role === 'model' ? 'model' : 'user', text });
    totalChars += text.length;
    if (totalChars > 8000) break; // batasi ukuran konteks
  }

  // Gemini menuntut percakapan diawali role "user"
  while (turns.length && turns[0].role !== 'user') {
    turns.shift();
  }

  // Gabungkan turn berurutan dengan role sama (hindari duplikat role berurutan)
  const contents = [];
